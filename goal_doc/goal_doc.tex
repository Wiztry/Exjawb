...
\begin{itemize}
	...
	\item[Laptop] For use when we need to run tests that cannot be performed by
		the desk on the small test platform.
	\item[Test platforms] Two test platforms will be available during the course
		of the master thesis work, a small scale test platform (denoted as Robot)
		and a full scale test platform (denoted as Mule).
		\begin{itemize}
			\item[\texttt{Robot}] This is the smal scale test platform. This will be
				available at all time, no planning and coordination with the rest of the
				team at Uniti will be needed. This can even be used when tests on out of
				office locations are to be performed.
			\item[\texttt{Mule}] This is the full scale test platform. This will be
				used by the rest of the development team of Uniti, and we will need to
				plan and coordinate usage of this test platform with the rest of the
				team at Uniti.
		\end{itemize}
	\item[\texttt{TOBY-L210}] Long range radio module. 4G with 3G/2G fallback.
	\item[\texttt{EMMY-w163}] Short range radio module. WiFi.
	\item[\texttt{EVK-L2x}] Evaluation kit for \texttt{TOBY-L2x}
	\item[\texttt{EVK-EMMY-W1}] Evaluation kit for \texttt{EMMY-w1x}
...
\signature{Uniti supervisor}{Kristofer Jansson}\\
